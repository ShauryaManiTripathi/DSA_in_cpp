...
void solve() {
    ...
    set<pii> kingcheckpos;
    for(auto [dx, dy] : dir) {
        kingcheckpos.insert({xk + a*dx, yk + b*dy});
        kingcheckpos.insert({xk + b*dx, yk + a*dy});
    }
    int count = 0;
    for(auto [dx,dy]:dir){
        if(kingcheckpos.find({xq+a*dx, yq+b*dy}) != kingcheckpos.end()){
            count++;
        }
        if(a!=b)
        if(kingcheckpos.find({xq+b*dx, yq+a*dy}) != kingcheckpos.end()){
            count++;
        }
    }
    writeln(count);
