...
void solve() {
    ...
    read(s);
    int n = sz(s);
    // prefix no of zeroes and ones
    vi prefix_zeroes(n + 1, 0), prefix_ones(n + 1, 0);
    for(int i = 0; i < n; i++) {
        prefix_zeroes[i + 1] = prefix_zeroes[i] + (s[i] == '0');
        prefix_ones[i + 1] = prefix_ones[i] + (s[i] == '1');
    }
    int min_cost = 1000000;
    // start a for loop from n to 0
    for(int i = n; i >= 0; i--) {
        min_cost = min(min_cost,((prefix_zeroes[n]-prefix_ones[i]>=0)?prefix_zeroes[n]-prefix_ones[i]:1000000)+(prefix_ones[n]-prefix_zeroes[i]>=0?prefix_ones[n]-prefix_zeroes[i]:1000000));
    }
    writeln(min_cost);
