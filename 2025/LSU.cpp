...
void solve()
{
    ...
    for (int learned_languages = 0; learned_languages < (1 << 5); learned_languages++) {
        int current_cost = 0;
        int translation_uses = 0;
        for (int lang = 0; lang < 5; lang++) {
            if (!(learned_languages & (1 << lang))) {
                for (int j = 0; j < freq[lang]; j++) {
                    translation_uses++;
                    current_cost += translation_uses;
                }
            }
            else if (freq[lang] > 0) {
                current_cost += c;
