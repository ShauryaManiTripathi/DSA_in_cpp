...
void solve() {
    ...
    // record next‐occurrence index for each position
    const int INF = n + 5;
    
    // Find max element to size the arrays properly
    int max_val = *max_element(a.begin(), a.end());
    
    vector<int> last(max_val + 1, INF), nxt(n, INF);
    for (int i = n-1; i >= 0; --i) {
        nxt[i] = last[a[i]];
        last[a[i]] = i;
    }
    ...
    int ans = 0;
    int mn = INF;
    // greedily cut whenever all elements seen since last cut reappear later
    for (int i = 0; i < n; ++i) {
        mn = min(mn, nxt[i]);
        if (mn > i) {
            ++ans;
