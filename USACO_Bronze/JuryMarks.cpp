...
void solve() {
    int k,n;
    read(k,n);
    vector<int> rating(k), scores(n);
    read(rating, scores);

    vector<int> prefixSum(k);
    prefixSum[0] = rating[0];
    for(int i=1; i<k; i++) {
        prefixSum[i] = prefixSum[i-1] + rating[i];
    }

    set<int> scoresBeginning;
    for(int i=0; i<k; i++) {
        scoresBeginning.insert(scores[0] - prefixSum[i]);
    }

    int ans = 0;
    for(int start: scoresBeginning) {
        set<int> points;
        for(int i=0; i<k; i++) {
            points.insert(start + prefixSum[i]);
        }
        bool valid = true;
        for(int p: scores) {
            if(!points.count(p)) { valid = false; break; }
        }
        ans += valid;
    }
