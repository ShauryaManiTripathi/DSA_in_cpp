...
void solve() {
    ...
    // Count positions where elements are different
    int diff_count = 0;
    for(int i = 0; i < n; i++) {
        if(a[i] != sorted_a[i]) {
            diff_count++;
    ...
    // The answer is the number of different positions minus 1
    // This is because when Bessie moves out of place, she affects
    // her original position and her new position, but we only need
    // to count the net displacement
    cout << max(0, diff_count - 1) << endl;
}
